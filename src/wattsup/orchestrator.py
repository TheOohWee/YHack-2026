...
def run_energy_poll(user_id: str, settings: Settings, *, dry_run: bool = False) -> PollContext:
    ...
    if not g_res.ok:
        carried: FuelMix | None = None
        try:
            carried = fetch_latest_good_fuel_mix(settings, user_id)
        except Exception:
            carried = None
        if carried is not None:
            ctx.fuel_mix = carried
            if ctx.renewable_pct is None:
                ctx.renewable_pct = min(
                    100.0,
                    carried.wind + carried.solar + carried.battery_storage,
                )
        else:
    ...
    coal_history: list[float] = []
    grid_stress_z: float | None = None
    if coll is not None and not dry_run:
        coal_history = fetch_recent_coal_pcts(coll, user_id, settings.zscore_window)
        grid_stress_z = z_score(coal_history, float(fuel.coal))
