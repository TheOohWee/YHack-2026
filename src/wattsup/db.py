...
def _coal_pct_from_row(row: dict[str, Any]) -> float | None:
    raw = row.get("fuel_mix")
...
def fetch_recent_coal_pcts(collection: Collection, user_id: str, limit: int) -> list[float]:
    """Chronological coal % samples (oldest first), same window as z-score history."""
    cursor = (
        collection.find({"user_id": user_id, "fuel_mix": {"$exists": True, "$ne": None}})
        .sort("timestamp", DESCENDING)
        .limit(limit)
    )
    out: list[float] = []
    for row in cursor:
        p = _coal_pct_from_row(row)
        if p is not None:
            out.append(p)
