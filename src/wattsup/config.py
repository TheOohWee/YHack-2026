...
class Settings(BaseSettings):
    ...
    streak_grid_stress_sigma: float = Field(
        default=2.0,
        validation_alias="STREAK_GRID_STRESS_SIGMA",
        description=(
            "Coal-mix z-score at or above this counts as a 'dirty grid' spike for passive streak credit "
            "(when efficiency holds vs prior poll)."
        ),
    )
    streak_hold_tolerance: float = Field(
        default=5.0,
        validation_alias="STREAK_HOLD_TOLERANCE",
        description="Max eco-score drop vs prior poll while still crediting a passive 'clean day' during grid stress.",
    )
