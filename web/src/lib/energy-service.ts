  ...
  return {
    ...
    renewable_pct:
      typeof doc.renewable_pct === "number"
        ? doc.renewable_pct
        : undefined,
    eco_efficiency_score: toFiniteNumber(doc.eco_efficiency_score),
    z_score: toFiniteNumber(doc.z_score) ?? undefined,
    gridstatus_ok:
...
export async function getEnergySnapshot(userId: string): Promise<EnergySnapshot> {
  ...
  const ecoLatest =
    toFiniteNumber(latestDoc?.eco_efficiency_score) ??
    latest?.eco_efficiency_score ??
    null;
  const dialPercent = normalizeDialPercent(logs, ecoLatest, latest);
